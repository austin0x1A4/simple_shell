...
/**
 * hsh - Main shell loop that handles command execution.
...
/**
 * find_builtin - Finds and executes a builtin command, if present.
 * @info: Pointer to the parameter and return info struct.
 *
 * Return: -1 if builtin not found, 0 if executed successfully,
 * 1 if builtin found but not successful, or 2 if signals exit().
 */
