...
/**
 * write_history_to_file - writes the history list to a file
 * @info: pointer to the parameter struct
 *
...
/**
 * read_history_from_file - reads history from the history file
 * @info: pointer to the parameter struct
 *
 * Return: Number of history entries read from the file, 0 on failure.
 */
 ...
 * @linecount: the linecount for the history entry
 *
...
int build_history_list(info_t *info, char *buf, int linecount)
{
	...
	if (info->history)
		node = info->history;
	if (add_node_end(&node, buf, linecount) == NULL)
		return (-1);
	if (!info->history)
		info->history = node;
	return (0);
